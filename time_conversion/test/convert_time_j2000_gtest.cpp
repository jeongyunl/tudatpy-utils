...
{
	EXPECT_DOUBLE_EQ(TimeConverter::instance().utc_iso_to_utc_j2000("2000-01-01T12:00:00Z"), 0.0);
}

TEST(ConvertTimeJ2000, TaiJ2000EpochIsZero)
{
	EXPECT_DOUBLE_EQ(TimeConverter::instance().utc_iso_to_tai_j2000("2000-01-01T11:59:28Z"), 0.0);
}

TEST(ConvertTimeJ2000, TimezoneOffsetIsApplied)
{
	// 12:00:00+01:00 == 11:00:00Z, so should be -3600 seconds from UTC J2000 epoch.
	EXPECT_DOUBLE_EQ(TimeConverter::instance().utc_iso_to_utc_j2000("2000-01-01T12:00:00+01:00"), -3600.0);
}
...
TEST(ConvertTimeJ2000, LeapSecondIsHandledAndDoesNotCountTransitionYet)
{
	...
	const double t_leap = TimeConverter::instance().utc_iso_to_tai_j2000("2016-12-31T23:59:60Z");
	const double t_next = TimeConverter::instance().utc_iso_to_tai_j2000("2017-01-01T00:00:00Z");
	EXPECT_DOUBLE_EQ(t_next - t_leap, 1.0);
}

TEST(ConvertTimeJ2000, Iso8601EqualTreatsTSeparatorAsOptional)
{
	EXPECT_TRUE(TimeConverter::instance().iso_8601_equal("2000-01-01T12:00:00Z", "2000-01-01 12:00:00Z", 3));
}
...
TEST(ConvertTimeJ2000, Iso8601EqualRespectsFractionalPrecision)
{
	...
	EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(a, b, 3));
	// At microsecond precision they should not.
	EXPECT_FALSE(TimeConverter::instance().iso_8601_equal(a, b, 6));
}

TEST(ConvertTimeJ2000, UtcAndTaiJ2000AreFinite)
{
	const double utc = TimeConverter::instance().utc_iso_to_utc_j2000("2020-01-01T00:00:00Z");
	const double tai = TimeConverter::instance().utc_iso_to_tai_j2000("2020-01-01T00:00:00Z");
...
TEST(ConvertTimeJ2000, IsoToJ2000MatchesReferenceData)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		...
		{
			const auto utc_iso_from_j2000 = TimeConverter::instance().utc_j2000_to_utc_iso(record.utc);
			EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, utc_iso_from_j2000, 3))
...
TEST(ConvertTimeJ2000, UtcJ2000ToParsedRoundTrip)
{
	...
	const double utc_j2000_time = TimeConverter::instance().utc_iso_to_utc_j2000(iso_string);
	const ParsedUtcIso parsed = TimeConverter::instance().utc_j2000_to_parsed_utc_iso(utc_j2000_time);
...
TEST(ConvertTimeJ2000, TaiJ2000ToParsedRoundTrip)
{
	...
	const double tai_j2000_time = TimeConverter::instance().utc_iso_to_tai_j2000(iso_string);
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000AtEpoch)
{
	...
	EXPECT_DOUBLE_EQ(TimeConverter::instance().posix_to_tai_j2000(tai_j2000_epoch_posix), 0.0);
}
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000ConsistentWithUtcIsoPath)
{
	...
	const double posix_time = TimeConverter::instance().utc_iso_to_posix(iso_string);
	const double tai_j2000_direct = TimeConverter::instance().posix_to_tai_j2000(posix_time);
	const double tai_j2000_iso_path = TimeConverter::instance().utc_iso_to_tai_j2000(iso_string);
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000WithEpochRecords)
{
	...
	for(const auto& record : convert_time_test::epoch_records())
	{
		...
		{ // Convert POSIX to TAI J2000
			const double tai_j2000_time = TimeConverter::instance().posix_to_tai_j2000(record.posix);
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000RoundTripViaIso)
{
	...
	const double posix_time = TimeConverter::instance().utc_iso_to_posix(original_iso);
	const double tai_j2000_time = TimeConverter::instance().posix_to_tai_j2000(posix_time);

	// Convert back: TAI J2000 -> parsed -> ISO
	const ParsedUtcIso parsed = TimeConverter::instance().tai_j2000_to_parsed_utc_iso(tai_j2000_time);
	const double tai_j2000_back = TimeConverter::instance().parsed_utc_iso_to_tai_j2000(parsed);
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000BeforeEpoch)
{
	...
	const double tai_j2000_iso = TimeConverter::instance().utc_iso_to_tai_j2000(iso_string);
...
TEST(ConvertTimeJ2000, TaiJ2000ToPosixAtEpoch)
{
	...
	const double posix_time = TimeConverter::instance().tai_j2000_to_posix(0.0);
...
TEST(ConvertTimeJ2000, TaiJ2000ToPosixConsistentWithUtcIsoPath)
{
	...
	const double posix_direct = TimeConverter::instance().tai_j2000_to_posix(tai_j2000_time);
	const double posix_iso_path = TimeConverter::instance().utc_iso_to_posix(iso_string);
...
TEST(ConvertTimeJ2000, TaiJ2000ToPosixWithEpochRecords)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		if(record.posix >= epochs::UTC_1972_EPOCH_IN_POSIX_TIME)
		{
			const double posix_from_tai = TimeConverter::instance().tai_j2000_to_posix(record.tai);
