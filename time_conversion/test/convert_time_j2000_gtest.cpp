...
{
	EXPECT_DOUBLE_EQ(TimeConverterBase::instance().utc_iso_to_utc_j2000("2000-01-01T12:00:00Z"), 0.0);
}

TEST(ConvertTimeJ2000, TaiJ2000EpochIsZero)
{
	EXPECT_DOUBLE_EQ(TimeConverterBase::instance().utc_iso_to_tai_j2000("2000-01-01T11:59:28Z"), 0.0);
}
...
TEST(ConvertTimeJ2000, TimezoneOffsetIsApplied)
{
	...
	EXPECT_DOUBLE_EQ(
		TimeConverterBase::instance().utc_iso_to_utc_j2000("2000-01-01T12:00:00+01:00"),
		-3600.0
	);
}
...
TEST(ConvertTimeJ2000, LeapSecondIsHandledAndDoesNotCountTransitionYet)
{
	...
	const double t_leap = TimeConverterBase::instance().utc_iso_to_tai_j2000("2016-12-31T23:59:60Z");
	const double t_next = TimeConverterBase::instance().utc_iso_to_tai_j2000("2017-01-01T00:00:00Z");
...
{
	EXPECT_TRUE(
		TimeConverterBase::instance().iso_8601_equal("2000-01-01T12:00:00Z", "2000-01-01 12:00:00Z", 3)
	);
}
...
{
	EXPECT_FALSE(
		TimeConverterBase::instance().iso_8601_equal("2000-01-01T12:00:00Z", "2000-01-01T12:00:00Z", 10)
	);
}
...
TEST(ConvertTimeJ2000, Iso8601EqualRespectsFractionalPrecision)
{
	...
	EXPECT_TRUE(TimeConverterBase::instance().iso_8601_equal(a, b, 3));
	// At microsecond precision they should not.
	EXPECT_FALSE(TimeConverterBase::instance().iso_8601_equal(a, b, 6));
}

TEST(ConvertTimeJ2000, UtcAndTaiJ2000AreFinite)
{
	const double utc = TimeConverterBase::instance().utc_iso_to_utc_j2000("2020-01-01T00:00:00Z");
	const double tai = TimeConverterBase::instance().utc_iso_to_tai_j2000("2020-01-01T00:00:00Z");
	...
	for(const auto& record : convert_time_test::epoch_records())
	{
															  ...
															  &TimeConverterTudat::instance() })
		{
			const ParsedUtcIso parsed_utc_iso =
				TimeConverterBase::instance().utc_iso_to_parsed_utc_iso(record.iso
...
TEST(ConvertTimeJ2000, UtcJ2000ToParsedRoundTrip)
{
	...
	const double utc_j2000_time = TimeConverterBase::instance().utc_iso_to_utc_j2000(iso_string);
	const ParsedUtcIso parsed = TimeConverterBase::instance().utc_j2000_to_parsed_utc_iso(utc_j2000_time);
...
TEST(ConvertTimeJ2000, TaiJ2000ToParsedRoundTrip)
{
	...
	const double tai_j2000_time = TimeConverterBase::instance().utc_iso_to_tai_j2000(iso_string);
	const ParsedUtcIso parsed = TimeConverterBase::instance().tai_j2000_to_parsed_utc_iso(tai_j2000_time);
...
TEST(ConvertTimeJ2000, TaiJ2000ToParsedReturnsLeapSecondLabel)
{
	const double tai_j2000_time =
		TimeConverterBase::instance().utc_iso_to_tai_j2000("2016-12-31T23:59:60.250000000Z");
	const ParsedUtcIso parsed = TimeConverterBase::instance().tai_j2000_to_parsed_utc_iso(tai_j2000_time);
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000AtEpoch)
{
	...
	EXPECT_DOUBLE_EQ(TimeConverterBase::instance().posix_to_tai_j2000(tai_j2000_epoch_posix), 0.0);
}
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000ConsistentWithUtcIsoPath)
{
	...
	const double posix_time = TimeConverterBase::instance().utc_iso_to_posix(iso_string);
	const double tai_j2000_direct = TimeConverterBase::instance().posix_to_tai_j2000(posix_time);
	const double tai_j2000_iso_path = TimeConverterBase::instance().utc_iso_to_tai_j2000(iso_string);
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000RoundTripViaIso)
{
	...
	const double posix_time = TimeConverterBase::instance().utc_iso_to_posix(original_iso);
	const double tai_j2000_time = TimeConverterBase::instance().posix_to_tai_j2000(posix_time);

	// Convert back: TAI J2000 -> parsed -> ISO
	const ParsedUtcIso parsed = TimeConverterBase::instance().tai_j2000_to_parsed_utc_iso(tai_j2000_time);
	const double tai_j2000_back = TimeConverterBase::instance().parsed_utc_iso_to_tai_j2000(parsed);
...
TEST(ConvertTimeJ2000, PosixToTaiJ2000BeforeEpoch)
{
	...
	const double tai_j2000_iso = TimeConverterBase::instance().utc_iso_to_tai_j2000(iso_string);
...
TEST(ConvertTimeJ2000, TaiJ2000ToPosixAtEpoch)
{
	...
	const double posix_time = TimeConverterBase::instance().tai_j2000_to_posix(0.0);
...
TEST(ConvertTimeJ2000, TaiJ2000ToPosixConsistentWithUtcIsoPath)
{
	...
	const double posix_direct = TimeConverterBase::instance().tai_j2000_to_posix(tai_j2000_time);
	const double posix_iso_path = TimeConverterBase::instance().utc_iso_to_posix(iso_string);
...
TEST(ConvertTimeJ2000, TaiJ2000ToPosixWithEpochRecords)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		if(record.posix >= epochs::UTC_1972_EPOCH_IN_POSIX_TIME)
		{
			const double posix_from_tai = TimeConverterBase::instance().tai_j2000_to_posix(record.tai);
