...
TEST_F(ConvertTimeDataDrivenTest, IsoToAllNumericScalesMatchReferenceData)
{
	...
	{
		EXPECT_NEAR(TimeConverter::instance().utc_iso_to_posix(record.iso), record.posix, convert_time_test::kTolExactLike)
			<< record.iso;
		...
		{
			EXPECT_NEAR(TimeConverter::instance().utc_iso_to_tai_j2000(record.iso), record.tai, convert_time_test::kTolTimeScale)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().utc_iso_to_tt_j2000(record.iso), record.tt, convert_time_test::kTolTimeScale)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().utc_iso_to_tdb_j2000(record.iso), record.tdb, convert_time_test::kTolTdb)
				<< record.iso;
		}
		...
		const auto tai_time = TimeConverter::instance().utc_iso_to_tai_time(record.iso);
		const auto iso_from_tai = std::format("{:%F %T}", std::chrono::tai_clock::to_utc(tai_time));
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_tai, 3)) << record.iso << " != " << iso_from_tai;
#endif
	}
}
...
TEST_F(ConvertTimeDataDrivenTest, PosixToOtherScalesMatchReferenceData)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		...
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(iso_from_posix, record.iso, 3)) << iso_from_posix << " != " << record.iso;

		EXPECT_NEAR(TimeConverter::instance().posix_to_utc_j2000(record.posix), record.utc, convert_time_test::kTolExactLike)
			<< record.iso;

		if(record.posix >= epochs::UTC_1972_EPOCH_IN_POSIX_TIME)
		{
			EXPECT_NEAR(TimeConverter::instance().posix_to_tai_j2000(record.posix), record.tai, convert_time_test::kTolExactLike)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().posix_to_tt_j2000(record.posix), record.tt, convert_time_test::kTolExactLike)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().posix_to_tdb_j2000(record.posix), record.tdb, convert_time_test::kTolTdb)
				<< record.iso;
		}
		...
		const auto utc_time = TimeConverter::instance().posix_to_utc_time(record.posix);
		const auto iso_from_utc = std::format("{:%F %T}", utc_time);
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_utc, 3)) << record.iso << " != " << iso_from_utc;
#endif

#ifdef HAS_CHRONO_TAI_CLOCK
		const auto tai_time = TimeConverter::instance().posix_to_tai_time(record.posix);
		const auto iso_from_tai = std::format("{:%F %T}", std::chrono::tai_clock::to_utc(tai_time));
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_tai, 3)) << record.iso << " != " << iso_from_tai;
#endif
	}
...
TEST_F(ConvertTimeDataDrivenTest, UtcToOtherScalesMatchReferenceData)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		...
		EXPECT_NEAR(TimeConverter::instance().utc_j2000_to_posix(record.utc), record.posix, convert_time_test::kTolExactLike)
			<< record.iso;

		if(record.posix >= epochs::UTC_1972_EPOCH_IN_POSIX_TIME)
		{
			EXPECT_NEAR(TimeConverter::instance().utc_j2000_to_tai_j2000(record.utc), record.tai, convert_time_test::kTolExactLike)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().utc_j2000_to_tt_j2000(record.utc), record.tt, convert_time_test::kTolExactLike)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().utc_j2000_to_tdb_j2000(record.utc), record.tdb, convert_time_test::kTolTdb)
				<< record.iso;
		}
...
#ifdef HAS_CHRONO_UTC_CLOCK
		const auto utc_time = TimeConverter::instance().utc_j2000_to_utc_time(record.utc);
		const auto iso_from_utc = std::format("{:%F %T}", utc_time);
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_utc, 3)) << record.iso << " != " << iso_from_utc;
#endif

#ifdef HAS_CHRONO_TAI_CLOCK
		const auto tai_time = TimeConverter::instance().utc_j2000_to_tai_time(record.utc);
		const auto iso_from_tai = std::format("{:%F %T}", std::chrono::tai_clock::to_utc(tai_time));
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_tai, 3)) << record.iso << " != " << iso_from_tai;
#endif
	}
...
TEST_F(ConvertTimeDataDrivenTest, TaiToOtherScalesMatchReferenceData)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		if(record.posix >= epochs::UTC_1972_EPOCH_IN_POSIX_TIME)
		{
			EXPECT_NEAR(TimeConverter::instance().tai_j2000_to_posix(record.tai), record.posix, convert_time_test::kTolExactLike)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().tai_j2000_to_utc_j2000(record.tai), record.utc, convert_time_test::kTolExactLike)
				<< record.iso;
		}
		EXPECT_NEAR(TimeConverter::instance().tai_j2000_to_tt_j2000(record.tai), record.tt, convert_time_test::kTolExactLike)
			<< record.iso;
		EXPECT_NEAR(TimeConverter::instance().tai_j2000_to_tdb_j2000(record.tai), record.tdb, convert_time_test::kTolTdb) << record.iso;
...
#ifdef HAS_CHRONO_UTC_CLOCK
			const auto utc_time = TimeConverter::instance().tai_j2000_to_utc_time(record.tai);
			const auto iso_from_utc = std::format("{:%F %T}", utc_time);
			EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_utc, 3)) << record.iso << " != " << iso_from_utc;
#endif
		}
...
TEST_F(ConvertTimeDataDrivenTest, TtToOtherScalesMatchReferenceData)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		if(record.posix >= epochs::UTC_1972_EPOCH_IN_POSIX_TIME)
		{
			EXPECT_NEAR(TimeConverter::instance().tt_j2000_to_posix(record.tt), record.posix, convert_time_test::kTolExactLike)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().tt_j2000_to_utc_j2000(record.tt), record.utc, convert_time_test::kTolExactLike)
				<< record.iso;
		}
		EXPECT_NEAR(TimeConverter::instance().tt_j2000_to_tai_j2000(record.tt), record.tai, convert_time_test::kTolExactLike)
			<< record.iso;
		EXPECT_NEAR(TimeConverter::instance().tt_j2000_to_tdb_j2000(record.tt), record.tdb, convert_time_test::kTolTdb) << record.iso;
...
#ifdef HAS_CHRONO_UTC_CLOCK
			const auto utc_time = TimeConverter::instance().tt_j2000_to_utc_time(record.tt);
			const auto iso_from_utc = std::format("{:%F %T}", utc_time);
			EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_utc, 3)) << record.iso << " != " << iso_from_utc;
#endif
		}
...
TEST_F(ConvertTimeDataDrivenTest, TdbToOtherScalesMatchReferenceData)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		if(record.posix >= epochs::UTC_1972_EPOCH_IN_POSIX_TIME)
		{
			EXPECT_NEAR(TimeConverter::instance().tdb_j2000_to_posix(record.tdb), record.posix, convert_time_test::kTolTdb)
				<< record.iso;
			EXPECT_NEAR(TimeConverter::instance().tdb_j2000_to_utc_j2000(record.tdb), record.utc, convert_time_test::kTolTdb)
				<< record.iso;
		}
		EXPECT_NEAR(TimeConverter::instance().tdb_j2000_to_tai_j2000(record.tdb), record.tai, convert_time_test::kTolTdb) << record.iso;
		EXPECT_NEAR(TimeConverter::instance().tdb_j2000_to_tt_j2000(record.tdb), record.tt, convert_time_test::kTolTdb) << record.iso;
...
#ifdef HAS_CHRONO_UTC_CLOCK
			const auto utc_time = TimeConverter::instance().tdb_j2000_to_utc_time(record.tdb);
			const auto iso_from_utc = std::format("{:%F %T}", utc_time);
			EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(record.iso, iso_from_utc, 3)) << record.iso << " != " << iso_from_utc;
#endif
		}
...
TEST_F(ConvertTimeDataDrivenTest, UtcIsoIdentityRoundTripForRecords)
{
	for(const auto& record : convert_time_test::epoch_records())
	{
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(TimeConverter::instance().utc_iso_to_utc_iso(record.iso), record.iso, 3)) << record.iso;
	}
}

TEST(ConvertTimeIso, Iso8601EqualTreatsTSeparatorAsOptional)
{
	EXPECT_TRUE(TimeConverter::instance().iso_8601_equal("1970-01-01T00:00:00.000000", "1970-01-01 00:00:00.000", 3));
	EXPECT_TRUE(TimeConverter::instance().iso_8601_equal("1970-01-01T00:00:00.000000", "1970-01-01 00:00:00.000", 6));
}

TEST(ConvertTimeIso, Iso8601EqualUsesRequestedFractionalPrecision)
{
	EXPECT_TRUE(TimeConverter::instance().iso_8601_equal("1970-01-01T00:00:00.123456", "1970-01-01 00:00:00.123000", 3));
	EXPECT_FALSE(TimeConverter::instance().iso_8601_equal("1970-01-01T00:00:00.123456", "1970-01-01 00:00:00.123000", 4));
	EXPECT_TRUE(TimeConverter::instance().iso_8601_equal("1970-01-01T00:00:00.9", "1970-01-01 00:00:00.900000", 6));
}
...
TEST(ConvertTimeIso, Iso8601EqualReturnsFalseForInvalidInputOrPrecision)
{
	...
	EXPECT_FALSE(TimeConverter::instance().iso_8601_equal("1970-01-01T00:00:00.000", "1970-01-01 00:00:00.000", 10));
}
...
TEST(ConvertTimeChrono, UtcPosixToUtcTimeSupportsCustomDuration)
{
	using namespace std::chrono;
	const double posix = 42.0;
	const auto t = TimeConverter::instance().posix_to_utc_time<milliseconds>(posix);
...
TEST(ConvertTimeChrono, UtcIsoToUtcTimePreservesLeapSeconds)
{
	...
	for(const auto& record : convert_time_test::epoch_records())
	{
		...
		// leap seconds.
			const auto t = TimeConverter::instance().utc_iso_to_utc_time<milliseconds>(record.iso);
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(TimeConverter::instance().utc_time_to_utc_iso(t), record.iso, 3)) << record.iso;
	}
}
...
TEST(ConvertTimeChrono, ParsedUtcIsoToUtcTimePreservesLeapSeconds)
{
	...
	for(const auto& record : convert_time_test::epoch_records())
	{
		const ParsedUtcIso parsed = TimeConverter::instance().utc_iso_to_parsed_utc_iso(record.iso);
			const auto t = TimeConverter::instance().parsed_utc_iso_to_utc_time<milliseconds>(parsed);
		EXPECT_TRUE(TimeConverter::instance().iso_8601_equal(TimeConverter::instance().utc_time_to_utc_iso(t), record.iso, 3)) << record.iso;
	}
}
...
TEST(ConvertTimeChrono, UtcTimeToUtcIsoFormatsWithoutTimezoneSuffix)
{
	...
	for(const auto& tc : cases)
	{
		EXPECT_EQ(TimeConverter::instance().utc_time_to_utc_iso(tc.input, true), tc.expected);
	}
}
#endif
