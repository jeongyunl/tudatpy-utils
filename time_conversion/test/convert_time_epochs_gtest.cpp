...
{
	EXPECT_EQ(epochs::UTC_J2000_EPOCH_IN_TAI_J2000_TIME, static_cast<std::int64_t>(J2000_TAI_MINUS_UTC));
	EXPECT_DOUBLE_EQ(
		epochs::UTC_J2000_EPOCH_IN_TT_J2000_TIME,
		J2000_TAI_MINUS_UTC + TT_EPOCH_MINUS_TAI_EPOCH
	);

	EXPECT_DOUBLE_EQ(epochs::TAI_J2000_EPOCH_IN_UTC_J2000_TIME, -J2000_TAI_MINUS_UTC);
	EXPECT_DOUBLE_EQ(epochs::TAI_J2000_EPOCH_IN_TT_J2000_TIME, TT_EPOCH_MINUS_TAI_EPOCH);

	EXPECT_DOUBLE_EQ(
		epochs::TT_J2000_EPOCH_IN_UTC_J2000_TIME,
		-J2000_TAI_MINUS_UTC - TT_EPOCH_MINUS_TAI_EPOCH
	);
	EXPECT_DOUBLE_EQ(epochs::TT_J2000_EPOCH_IN_TAI_J2000_TIME, -TT_EPOCH_MINUS_TAI_EPOCH);
}
...
TEST(ConvertTimeEpochs, PosixEpochOffsetsAreConsistent)
{
	...
	EXPECT_DOUBLE_EQ(
		...
		-static_cast<double>(epochs::UTC_J2000_EPOCH_IN_POSIX_TIME)
			+ static_cast<std::int64_t>(PRE_1972_TAI_MINUS_UTC_AT_1970)
	);
	EXPECT_DOUBLE_EQ(
		epochs::POSIX_EPOCH_IN_TT_J2000_TIME,
		epochs::POSIX_EPOCH_IN_TAI_J2000_TIME + TT_EPOCH_MINUS_TAI_EPOCH
	);
}
