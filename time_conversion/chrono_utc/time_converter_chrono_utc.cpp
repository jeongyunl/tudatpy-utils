...
TimeValue TimeConverterChronoUtc::convert_time(
...
double TimeConverterChronoUtc::posix_to_tai_j2000(double posix_time) const
{
#ifdef HAS_CHRONO_UTC_CLOCK
	...
	return std::chrono::duration<double>(
		...
		.count();
#else
	return TimeConverter::posix_to_tai_j2000(posix_time);
#endif
}

double TimeConverterChronoUtc::parsed_utc_iso_to_tai_j2000(const ParsedUtcIso& parsed_utc_iso) const
{
#ifdef HAS_CHRONO_UTC_CLOCK
	...
	return std::chrono::duration<double>(
		...
		.count();
#else
	return TimeConverter::parsed_utc_iso_to_tai_j2000(parsed_utc_iso);
#endif
}

ParsedUtcIso TimeConverterChronoUtc::tai_j2000_to_parsed_utc_iso(double tai_j2000_time) const
{
#ifdef HAS_CHRONO_UTC_CLOCK
...
#else
	return TimeConverter::tai_j2000_to_parsed_utc_iso(tai_j2000_time);
#endif
}