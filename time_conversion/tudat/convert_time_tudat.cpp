...
namespace
{
using TudatDispatchKey = std::pair<TimeFormat, TimeFormat>;
...
public:
	...
	TudatHandler() = default;

	template <typename Obj, typename Arg, typename Ret>
	TudatHandler(Ret (Obj::*func)(Arg) const)
	...
	template <typename Obj, typename Arg, typename Ret>
	TudatHandler(Ret (Obj::*func)(const Arg&) const)
		: callable_([func](const TimeValue& input_time, const TimeConverterTudat* tc_ptr) -> TimeValue {
			using BareArg = std::remove_cvref_t<Arg>;
...
std::map<TudatDispatchKey, TudatHandler> dispatchTable{
...
{
	TudatDispatchKey key{ input_format, output_format };
	auto it = dispatchTable.find(key);
	if(it != dispatchTable.end())
	{
		const TudatHandler& handler = it->second;
		if(handler.getInputType() == TudatHandler::InputType::STRING
		   && !std::holds_alternative<std::string>(input))
		{
			...
			handler.getInputType() == TudatHandler::InputType::DOUBLE
